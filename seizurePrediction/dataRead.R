...
trainFiles_patient1 = list.files(path = "./train_3/", pattern = "*.mat", full.names = F)
filenames = list()
j = 1
...
for(i in 1:length(trainFiles_patient1))
{
  if(filter$safe[grep(trainFiles_patient1[i], filter$image)] == 1)
    {
      filenames[j] = trainFiles_patient1[i]
      j = j + 1
    }
}
trainset = matrix(,length(filenames),(16*8 + 2))
trainset = as.data.frame(trainset)

expectedOutput = list()

colnames(trainset) = c("path","target",paste("min_",rep(1:16),sep=""),paste("max_",rep(1:16),sep=""),paste("mean_",rep(1:16),sep=""),paste("median_",rep(1:16),sep=""),paste("sem_",rep(1:16),sep=""),paste("abssum_",rep(1:16),sep=""), paste("skewness_",rep(1:16),sep=""), paste("kurtosis_",rep(1:16),sep=""))

for (i in 1: length(filenames)){

  data = readMat(paste("train_3/", filenames[i], sep = ""))
  ...
  trainset[i,] = c("train_3/", filenames[i], min_list, max_list, mean_list, median_list, sem_list, abssum_list, skewnessInData, kurtosisInData)

  expectedOutput[i] = filter$class[grep(trainFiles_patient1[i], filter$image)]
}

opFileName = "train_3_set.csv"
write.table(trainset, paste("features/", opFileName, sep = ""), quote=F, row.names=F, col.names=T, sep=",")

opClassFileName = "train_3_class.txt"
lapply(expectedOutput, cat, "\n", file=paste("features/", opClassFileName, sep = ""), append=TRUE)
